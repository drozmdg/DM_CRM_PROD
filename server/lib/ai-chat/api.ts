...
export const processMessage = async (message: string): Promise<string> => {
  try {
    ...
    // Force using fallback responses to ensure we get responses
    console.log('Using fallback response');
    const { getFallbackResponse } = await import('./responses');
    ...
    // Return error message
    return `Error processing your request: ${error instanceof Error ? error.message : 'Unknown error'}. Please try again later.`;
  }
};
