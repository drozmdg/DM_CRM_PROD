...
CREATE FUNCTION public.health_check() RETURNS TABLE(status text, check_timestamp timestamp without time zone, database_name text, tables_count integer, users_count integer)
    LANGUAGE plpgsql
    AS $$
BEGIN
    RETURN QUERY
    SELECT 
        ...
        NOW()::TIMESTAMP WITHOUT TIME ZONE as check_timestamp,
